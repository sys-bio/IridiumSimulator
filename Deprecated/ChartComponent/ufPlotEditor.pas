...
begin
  ...
  SpinNumberOfXMinorTicks.Value := Plot.GridStyle.MinorDivisions;
  SpinNumberOfYMinorTicks.Value := Plot.GridStyle.MinorDivisions;

// SpinNumberOfXMajorTicks.Value := Plot.GridStyle.NumberOfXTicks;
...
//  cboYAXisColor.Color := gp.YAxisColor;
  ...
  chkLogXAxes.IsChecked := Plot.LogX;
  chkLogYAxes.IsChecked := Plot.LogY;

  edtXMinimum.Text := floattostr (Plot.AxisProperties.MinX);
  edtXMaximum.Text := floattostr (Plot.AxisProperties.MaxX);
  edtYMinimum.Text := floattostr (Plot.AxisProperties.MinY);
  edtYMaximum.Text := floattostr (Plot.AxisProperties.MaxY);
...
//
  Plot.LogX := chkLogXAxes.IsChecked;
  Plot.LogY := chkLogYAxes.IsChecked;

  Plot.AxisProperties.MinX := strtofloat (edtXMinimum.Text);
  Plot.AxisProperties.MaxX := strtofloat (edtXMaximum.Text);
  Plot.AxisProperties.MinY := strtofloat (edtYMinimum.Text);
  Plot.AxisProperties.MaxY := strtofloat (edtYMaximum.Text);
...
begin
  ...
  Plot.GridStyle.MinorDivisions := trunc (SpinNumberOfXMinorTicks.Value);
  Plot.Redraw;
end;
...
begin
  ...
  Plot.GridStyle.MinorDivisions := trunc (SpinNumberOfYMinorTicks.Value);
  Plot.Redraw;
end;
...
begin
  ...
  if (Plot.AxisProperties.MinX <= 0) or
     (Plot.AxisProperties.MaxX <= 0) or
     (Plot.AxisProperties.MinY <= 0) or
     (Plot.AxisProperties.MaxY <= 0) then
     begin
     ...
     begin
     Plot.LogX := chkLogBothAxes.IsChecked;
     Plot.LogY := chkLogBothAxes.IsChecked;
     Plot.Redraw;
    end;
end;
...
begin
  ...
  if (Plot.AxisProperties.MinX <= 0) or
     (Plot.AxisProperties.MaxX <= 0) then
     begin
...
begin
  ...
  if (Plot.AxisProperties.MinY <= 0) or
     (Plot.AxisProperties.MaxY <= 0) then
     begin
