...
begin
  ...
  { Clear existing attributes BEFORE inherited runs SetText. The base
  TLinesLayout.ReplaceLine calls TTextLayout.SetText which triggers
  a re-measure (CreateCTFrame on macOS) that re-applies whatever
  attributes the layout already holds. Those attributes were sized
  for the previous line content and may now extend past the end of
  the new (shorter) text, which CoreText rejects with EAccessViolation
  inside AddColorAttribute. Windows/DirectWrite tolerates it; macOS
  does not. }
